...
$query = mysqli_query($conn, "SELECT * FROM `Instituicoes` WHERE email = '$email'");
$query2 = mysqli_query($conn, "SELECT pass FROM Instituicoes WHERE email='$email'");
$row = mysqli_fetch_assoc($query2);

if (mysqli_num_rows($query)) {
    $query = "SELECT `nome` FROM `Instituicoes` WHERE `email`='$email'";
